...
export function plannerSystem(availableSkills: string[]): string {
  return [
    ...
    `- tools per agent: list only NETWORK tools an agent needs, from: fetch, search, discover, email, telegram, webhook. Chiefs/managers usually need none.`,
    ...
    `   email/telegram/webhook need company connectors configured; telegram is typically for the chief ↔ owner channel.`,
    `   outbound email/telegram/webhook are often gated via policies.approveTools.)`,
